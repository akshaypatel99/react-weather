export const formatTime = (unixTime, timezoneOffset) => {
	return new Date((unixTime + timezoneOffset) * 1e3).toTimeString().slice(0, 5);
};

export const formatDate = (unixTime, timezoneOffset) => {
	return new Date((unixTime + timezoneOffset) * 1e3).toDateString();
};

export const formatDayDate = (unixTime, timezoneOffset) => {
	const day = new Date(
		(unixTime + timezoneOffset) * 1e3
	).toLocaleString('en-gb', { weekday: 'long' });
	let date = new Date((unixTime + timezoneOffset) * 1e3).getDate();
